...
public class SSRepo_MerchantTransactions implements SQLData {

	private final String SCHEMA_SQL = "OPENGATEV2";
    private final String PACKAGE_SQL = "opengate_acquirer";
    private final String PROCEDURE_SQL = "get_merchant_transactions3";
    @Getter @Setter
    
    private SSEnt_MerchantTransactionsRq request;
    ...
    public SSRepo_MerchantTransactions() {
        logr = new SS_LoggingImpl();
        logr.setClazz(getClass());
    }
 
    public SSRepo_MerchantTransactions(SSEnt_MerchantTransactionsRq request) {
        this();
    ...
    public void readSQL(SQLInput stream, String typeName) {
        try {
            logr.getLog().trace("Start readSQL, get merchant transactions 3");
            response = stream.readString(); // Read output parameter
        } catch (NullPointerException e) {
            logr.getLog().trace("No data available in get merchant transactions 3 : " + e.getMessage());
        } catch (Exception e) {
            logr.getLog().error("Error in readSQL, get merchant transactions 3", e);
        }
    }
    ...
    public void writeSQL(SQLOutput stream) throws SQLException {
        try {
            logr.getLog().trace("Start writeSQL, get merchant transactions 3");
            stream.writeString(request != null ? request.toString() : null); 
            logr.getLog().trace("End writeSQL, get merchant transactions 3 ");
        } catch (Exception e) {
            logr.getLog().error("Error in writeSQL, get merchant transactions 3", e);
        }
    }
    ...
    public String processing(DataSource dataSource) {
        ...
        try {
            logr.getLog().trace("Start processing, get merchant transactions 3 ");
            if (request == null) {
