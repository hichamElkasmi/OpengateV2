...
public class SSRepo_MerchantAppliValidation implements SQLData  {

	private final String SCHEMA_SQL = "OPENGATEV2";
    private final String PACKAGE_SQL = "opengate_acquirer";
    private final String PROCEDURE_SQL = "merchantapplicationvalidation";
    @Getter @Setter
    
    private SSEnt_MerchantAppliValidationRq request;
    ...
    public SSRepo_MerchantAppliValidation() {
        logr = new SS_LoggingImpl();
        logr.setClazz(getClass());
    }
 
    public SSRepo_MerchantAppliValidation(SSEnt_MerchantAppliValidationRq request) {
        this();
    ...
    public void readSQL(SQLInput stream, String typeName) {
        try {
            logr.getLog().trace("Start readSQL, merchant application validation ");
            response = stream.readString(); // Read output parameter
        } catch (NullPointerException e) {
            logr.getLog().trace("No data available in merchant application validation : " + e.getMessage());
        } catch (Exception e) {
            logr.getLog().error("Error in readSQL, merchant application validation ", e);
        }
    }
    ...
    public void writeSQL(SQLOutput stream) throws SQLException {
        try {
            logr.getLog().trace("Start writeSQL, merchant application validation ");
            stream.writeString(request != null ? request.toString() : null); 
            logr.getLog().trace("End writeSQL, merchant application validation ");
        } catch (Exception e) {
            logr.getLog().error("Error in writeSQL, merchant application validation  ", e);
        }
    }
    ...
    public String processing(DataSource dataSource) {
        ...
        try {
            logr.getLog().trace("Start processing, merchant application validation ");
            if (request == null) {
