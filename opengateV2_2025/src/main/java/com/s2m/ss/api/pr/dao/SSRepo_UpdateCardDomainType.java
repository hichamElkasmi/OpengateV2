...
public class SSRepo_UpdateCardDomainType implements SQLData {

	private final String SCHEMA_SQL = "OPENGATEV2";
    private final String PACKAGE_SQL = "OPENGATE_CARD_MANAGEMENT";
    private final String PROCEDURE_SQL = "update_card_domain_type";
    @Getter @Setter
    
    private SSEnt_UpdateCardDomainTypeRq request;
    ...
    public SSRepo_UpdateCardDomainType() {
        logr = new SS_LoggingImpl();
        logr.setClazz(getClass());
    }
 
    public SSRepo_UpdateCardDomainType(SSEnt_UpdateCardDomainTypeRq request) {
        this();
    ...
    public void readSQL(SQLInput stream, String typeName) {
        try {
            logr.getLog().trace("Start readSQL, update card domain type");
            response = stream.readString(); 
        } catch (NullPointerException e) {
            logr.getLog().trace("No data available update card domain type: " + e.getMessage());
        } catch (Exception e) {
            logr.getLog().error("Error in readSQL, update card domain type", e);
        }
    }
    ...
    public void writeSQL(SQLOutput stream) throws SQLException {
        try {
            logr.getLog().trace("Start writeSQL, update card domain type");
            stream.writeString(request != null ? request.toString() : null); 
            logr.getLog().trace("End writeSQL, update card domain type");
        } catch (Exception e) {
            logr.getLog().error("Error in writeSQL, update card domain type", e);
        }
    }
    ...
    public String processing(DataSource dataSource) {
        ...
        try {
            logr.getLog().trace("Start processing, update card domain type");
            if (request == null) {
