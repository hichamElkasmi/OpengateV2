...
public class SSrepo_validateCardPin implements SQLData {

    private final String SCHEMA_SQL = "OPENGATEV2";
    private final String PACKAGE_SQL = "OPENGATE_CARD_MANAGEMENT";
    private final String PROCEDURE_SQL = "VALIDATE_CARD_PIN1";
    @Getter @Setter
    
    private SSEnt_ValidateCardPinRq request;
    ...
    public SSrepo_validateCardPin() {
        logr = new SS_LoggingImpl();
        logr.setClazz(getClass());
    }
 
    public SSrepo_validateCardPin(SSEnt_ValidateCardPinRq request) {
        this();
    ...
    public void readSQL(SQLInput stream, String typeName) {
        try {
            logr.getLog().trace("Start readSQL, validate card pin ");
            response = stream.readString(); // Read output parameter
        } catch (NullPointerException e) {
            logr.getLog().trace("No data available in validate card pin : " + e.getMessage());
        } catch (Exception e) {
            logr.getLog().error("Error in readSQL, validate card pin ", e);
        }
    }
    ...
    public void writeSQL(SQLOutput stream) throws SQLException {
        try {
            logr.getLog().trace("Start writeSQL, validate card pin ");
            stream.writeString(request != null ? request.toString() : null); 
            logr.getLog().trace("End writeSQL, validate card pin ");
        } catch (Exception e) {
            logr.getLog().error("Error in writeSQL, validate card pin ", e);
        }
    }
    ...
    public String processing(DataSource dataSource) {
        ...
        try {
            logr.getLog().trace("Start processing, validate card pin ");
            if (request == null) {
