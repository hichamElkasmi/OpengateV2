...
public class SSRepo_RiskCardUpdate implements SQLData {

	private final String SCHEMA_SQL = "OPENGATEV2";
    private final String PACKAGE_SQL = "OPENGATE_CARD_MANAGEMENT";
    private final String PROCEDURE_SQL = "update_risk_card";
    @Getter @Setter
    
    private SSEnt_RiskCardUpdateRq request;
    ...
    public SSRepo_RiskCardUpdate() {
        logr = new SS_LoggingImpl();
        logr.setClazz(getClass());
    }
 
    public SSRepo_RiskCardUpdate(SSEnt_RiskCardUpdateRq request) {
        this();
    ...
    public void readSQL(SQLInput stream, String typeName) {
        try {
            logr.getLog().trace("Start readSQL, update risk card ");
            response = stream.readString(); // Read output parameter
        } catch (NullPointerException e) {
            logr.getLog().trace("No data available in update risk card : " + e.getMessage());
        } catch (Exception e) {
            logr.getLog().error("Error in readSQL, update risk card ", e);
        }
    }
    ...
    public void writeSQL(SQLOutput stream) throws SQLException {
        try {
            logr.getLog().trace("Start writeSQL, update risk card  ");
            stream.writeString(request != null ? request.toString() : null); 
            logr.getLog().trace("End writeSQL, update risk card ");
        } catch (Exception e) {
            logr.getLog().error("Error in writeSQL,  update risk card ", e);
        }
    }
    ...
    public String processing(DataSource dataSource) {
        ...
        try {
            logr.getLog().trace("Start processing, update risk card ");
            if (request == null) {
