...
public class SSRepo_ChangeMagPin implements SQLData {

	
	private final String SCHEMA_SQL = "OPENGATEV2";
    private final String PACKAGE_SQL = "OPENGATE_CARD_MANAGEMENT";
    private final String PROCEDURE_SQL = "CALL_CHANGE_MAG_PIN";
    @Getter @Setter
    
    private SSEnt_ChangeMagPinRq request;
    ...
    public SSRepo_ChangeMagPin() {
        logr = new SS_LoggingImpl();
        logr.setClazz(getClass());
    }
 
    public SSRepo_ChangeMagPin(SSEnt_ChangeMagPinRq request) {
        this();
    ...
    public void readSQL(SQLInput stream, String typeName) {
        try {
            logr.getLog().trace("Start readSQL, CHANGE MAG PIN");
            response = stream.readString(); // Read output parameter
        } catch (NullPointerException e) {
            logr.getLog().trace("No data available in CHANGE MAG PIN: " + e.getMessage());
        } catch (Exception e) {
            logr.getLog().error("Error in readSQL, CHANGE MAG PIN", e);
        }
    }
    ...
    public void writeSQL(SQLOutput stream) throws SQLException {
        try {
            logr.getLog().trace("Start writeSQL, CHANGE MAG PIN");
            stream.writeString(request != null ? request.toString() : null); 
            logr.getLog().trace("End writeSQL, change mag pin ");
        } catch (Exception e) {
            logr.getLog().error("Error in writeSQL, CHANGE MAG PIN", e);
        }
    }
    ...
    public String processing(DataSource dataSource) {
        ...
        try {
            logr.getLog().trace("Start processing, CHANGE MAG PIN");
            if (request == null) {
