...
public class SSRepo_InternetMailOrderStatus implements SQLData {
	private final String SCHEMA_SQL = "OPENGATEV2";
    private final String PACKAGE_SQL = "opengate_card_management";
    private final String PROCEDURE_SQL = "InternetMailOrder_status";
    @Getter @Setter
    
    private SSEnt_InternetMailOrderStatusRq request;
    ...
    public SSRepo_InternetMailOrderStatus() {
        logr = new SS_LoggingImpl();
        logr.setClazz(getClass());
    }
 
    public SSRepo_InternetMailOrderStatus(SSEnt_InternetMailOrderStatusRq request) {
        this();
    ...
    public void readSQL(SQLInput stream, String typeName) {
        try {
            logr.getLog().trace("Start readSQL, Internet Mail Order status");
            response = stream.readString(); 
        } catch (NullPointerException e) {
            logr.getLog().trace("No data available : " + e.getMessage());
        } catch (Exception e) {
            logr.getLog().error("Error in readSQL, Internet Mail Order status", e);
        }
    }
    ...
    public void writeSQL(SQLOutput stream) throws SQLException {
        try {
            logr.getLog().trace("Start writeSQL");
            stream.writeString(request != null ? request.toString() : null); 
            logr.getLog().trace("End writeSQL, Internet Mail Order status");
        } catch (Exception e) {
            logr.getLog().error("Error in writeSQL", e);
        }
    }
    ...
    public String processing(DataSource dataSource) {
        ...
        try {
            logr.getLog().trace("Start processing, Internet Mail Order status");
            if (request == null) {
