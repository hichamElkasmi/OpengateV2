...
public class SSRepo_UnLinkAccountToCard implements SQLData {

	private final String SCHEMA_SQL = "OPENGATEV2";
    private final String PACKAGE_SQL = "OPENGATE_CARD_MANAGEMENT";
    private final String PROCEDURE_SQL = "UNLINK_ACCOUNT_FROM_CARD";
    @Getter @Setter
    
    private SSEnt_UnLinkAccountToCardRq request;
    ...
    public SSRepo_UnLinkAccountToCard() {
        logr = new SS_LoggingImpl();
        logr.setClazz(getClass());
    }
 
    public SSRepo_UnLinkAccountToCard(SSEnt_UnLinkAccountToCardRq request) {
        this();
    ...
    public void readSQL(SQLInput stream, String typeName) {
        try {
            logr.getLog().trace("Start readSQL, unlink account to card ");
            response = stream.readString(); // Read output parameter
        } catch (NullPointerException e) {
            logr.getLog().trace("No data available unlink account from card : " + e.getMessage());
        } catch (Exception e) {
            logr.getLog().error("Error in readSQL, unlink account from card ", e);
        }
    }
    ...
    public void writeSQL(SQLOutput stream) throws SQLException {
        try {
            logr.getLog().trace("Start writeSQL, unlink account from card ");
            stream.writeString(request != null ? request.toString() : null); 
            logr.getLog().trace("End writeSQL, unlink account from card ");
        } catch (Exception e) {
            logr.getLog().error("Error in writeSQL, unlink account from card ", e);
        }
    }
    ...
    public String processing(DataSource dataSource) {
        ...
        try {
            logr.getLog().trace("Start processing, unlink account from card ");
            if (request == null) {
