...
public class SSRepo_SendPinByEmail implements SQLData {
	
	private final String SCHEMA_SQL = "OPENGATEV2";
    private final String PACKAGE_SQL = "opengate_card_management";
    private final String PROCEDURE_SQL = "SEND_PIN_BY_EMAIL";
    @Getter @Setter
    
    private SSEnt_SendPinByEmailRq request;
    ...
    public SSRepo_SendPinByEmail() {
        logr = new SS_LoggingImpl();
        logr.setClazz(getClass());
    }
 
    public SSRepo_SendPinByEmail(SSEnt_SendPinByEmailRq request) {
        this();
    ...
    public void writeSQL(SQLOutput stream) throws SQLException {
        try {
            logr.getLog().trace("Start writeSQL, send pin by email ");
            stream.writeString(request != null ? request.toString() : null); 
            logr.getLog().trace("End writeSQL, send pin by email ");
        } catch (Exception e) {
            logr.getLog().error("Error in writeSQL, send pin by email ", e);
        }
    }
    ...
    public String processing(DataSource dataSource) {
        ...
        try {
            logr.getLog().trace("Start processing, send pin by email ");
            if (request == null) {
