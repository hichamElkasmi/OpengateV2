...
public class SSRepo_cardlist implements SQLData {
	
	private final String SCHEMA_SQL = "OPENGATEV2";
    private final String PACKAGE_SQL = "opengate_card_management";
    private final String PROCEDURE_SQL = "card_list";
    @Getter @Setter
    
    private SSEnt_cardlistRq request;
    ...
    public SSRepo_cardlist() {
        logr = new SS_LoggingImpl();
        logr.setClazz(getClass());
    }
 
    public SSRepo_cardlist(SSEnt_cardlistRq request) {
        this();
    ...
    public void readSQL(SQLInput stream, String typeName) {
        try {
            logr.getLog().trace("Start readSQL, card list");
            response = stream.readString(); // Read output parameter
        } catch (NullPointerException e) {
            logr.getLog().trace("No data available in card list: " + e.getMessage());
        } catch (Exception e) {
            logr.getLog().error("Error in readSQL, card list", e);
        }
    }
    ...
    public void writeSQL(SQLOutput stream) throws SQLException {
        try {
            logr.getLog().trace("Start writeSQL, card list");
            stream.writeString(request != null ? request.toString() : null); 
            logr.getLog().trace("End writeSQL, card list ");
        } catch (Exception e) {
            logr.getLog().error("Error in writeSQL, card list", e);
        }
    }
    ...
    public String processing(DataSource dataSource) {
        ...
        try {
            logr.getLog().trace("Start processing, card list");
            if (request == null) {
