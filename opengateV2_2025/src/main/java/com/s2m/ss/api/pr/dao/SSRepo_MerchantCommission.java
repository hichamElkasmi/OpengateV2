...
public class SSRepo_MerchantCommission implements SQLData {

	
	private final String SCHEMA_SQL = "OPENGATEV2";
    private final String PACKAGE_SQL = "opengate_acquirer";
    private final String PROCEDURE_SQL = "update_merchant_commission";
    @Getter @Setter
    
    private SSEnt_MerchantCommissionRq request;
    ...
    public SSRepo_MerchantCommission() {
        logr = new SS_LoggingImpl();
        logr.setClazz(getClass());
    }
 
    public SSRepo_MerchantCommission(SSEnt_MerchantCommissionRq request) {
        this();
    ...
    public void readSQL(SQLInput stream, String typeName) {
        try {
            logr.getLog().trace("Start readSQL, update merchant commission ");
            response = stream.readString(); // Read output parameter
        } catch (NullPointerException e) {
            logr.getLog().trace("No data available in update merchant commision : " + e.getMessage());
        } catch (Exception e) {
    ...
    public void writeSQL(SQLOutput stream) throws SQLException {
        try {
            logr.getLog().trace("Start writeSQL, update merchant commission ");
            stream.writeString(request != null ? request.toString() : null); 
            logr.getLog().trace("End writeSQL, update merchant commission ");
        } catch (Exception e) {
            logr.getLog().error("Error in writeSQL, update merchant commission ", e);
        }
    }
    ...
    public String processing(DataSource dataSource) {
        ...
        try {
            logr.getLog().trace("Start processing, update merchant commission ");
            if (request == null) {
