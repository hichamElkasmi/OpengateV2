...
public class SSRepo_DebitApplication implements SQLData {

	
	private final String SCHEMA_SQL = "OPENGATEV2";
    private final String PACKAGE_SQL = "OPENGATE_CARD_MANAGEMENT";
    private final String PROCEDURE_SQL = "debitapplication";
    @Getter @Setter
    
    private SSEnt_DebitApplicationRq request;
    ...
    public SSRepo_DebitApplication() {
        logr = new SS_LoggingImpl();
        logr.setClazz(getClass());
    }
 
    public SSRepo_DebitApplication(SSEnt_DebitApplicationRq request) {
        this();
    ...
    public void readSQL(SQLInput stream, String typeName) {
        try {
            logr.getLog().trace("Start readSQL, debit application ");
            response = stream.readString(); // Read output parameter
        } catch (NullPointerException e) {
            logr.getLog().trace("No data available in debit application : " + e.getMessage());
        } catch (Exception e) {
            logr.getLog().error("Error in readSQL, debit application ", e);
        }
    }
    ...
    public void writeSQL(SQLOutput stream) throws SQLException {
        try {
            logr.getLog().trace("Start writeSQL, debit application ");
            stream.writeString(request != null ? request.toString() : null); 
            logr.getLog().trace("End writeSQL, debit application ");
        } catch (Exception e) {
            logr.getLog().error("Error in writeSQL, debit application", e);
        }
    }
    ...
    public String processing(DataSource dataSource) {
        ...
        try {
            logr.getLog().trace("Start processing, debit application");
            if (request == null) {
