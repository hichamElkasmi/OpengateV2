...
public class SSRepo_Replacement implements SQLData {

	private final String SCHEMA_SQL = "OPENGATEV2";
    private final String PACKAGE_SQL = "opengate_card_management";
    private final String PROCEDURE_SQL = "replacement";
    @Getter @Setter
    
    private SSEnt_ReplacementRq request;
    ...
    public SSRepo_Replacement() {
        logr = new SS_LoggingImpl();
        logr.setClazz(getClass());
    }
 
    public SSRepo_Replacement(SSEnt_ReplacementRq request) {
        this();
    ...
    public void readSQL(SQLInput stream, String typeName) {
        try {
            logr.getLog().trace("Start readSQL, replacement ");
            response = stream.readString(); // Read output parameter
        } catch (NullPointerException e) {
            logr.getLog().trace("No data available in replacement : " + e.getMessage());
        } catch (Exception e) {
            logr.getLog().error("Error in readSQL, replacement ", e);
        }
    }
    ...
    public void writeSQL(SQLOutput stream) throws SQLException {
        try {
            logr.getLog().trace("Start writeSQL, replacement ");
            stream.writeString(request != null ? request.toString() : null); 
            logr.getLog().trace("End writeSQL, replacement ");
        } catch (Exception e) {
            logr.getLog().error("Error in writeSQL, replacement ", e);
        }
    }
    ...
    public String processing(DataSource dataSource) {
        ...
        try {
            logr.getLog().trace("Start processing, replacement ");
            if (request == null) {
