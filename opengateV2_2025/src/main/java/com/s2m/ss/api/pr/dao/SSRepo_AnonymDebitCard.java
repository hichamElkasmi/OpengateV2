...
public class SSRepo_AnonymDebitCard implements SQLData {
	
	private final String SCHEMA_SQL = "OPENGATEV2";
    private final String PACKAGE_SQL = "opengate_card_management";
    private final String PROCEDURE_SQL = "anonymdebitcard";
    @Getter @Setter
    
    private SSEnt_AnonymDebitCardRq request;
    ...
    public SSRepo_AnonymDebitCard() {
        logr = new SS_LoggingImpl();
        logr.setClazz(getClass());
    }
 
    public SSRepo_AnonymDebitCard(SSEnt_AnonymDebitCardRq request) {
        this();
    ...
    public void readSQL(SQLInput stream, String typeName) {
        try {
            logr.getLog().trace("Start readSQL, anonym debit card");
            response = stream.readString(); // Read output parameter
        } catch (NullPointerException e) {
            logr.getLog().trace("No data available in anonym debit card: " + e.getMessage());
        } catch (Exception e) {
            logr.getLog().error("Error in readSQL, anonym debit card", e);
        }
    }
    ...
    public void writeSQL(SQLOutput stream) throws SQLException {
        try {
            logr.getLog().trace("Start writeSQL, anonym debit card");
            stream.writeString(request != null ? request.toString() : null); 
            logr.getLog().trace("End writeSQL, change status");
        } catch (Exception e) {
            logr.getLog().error("Error in writeSQL, anonym debit card", e);
        }
    }
    ...
    public String processing(DataSource dataSource) {
        ...
        try {
            logr.getLog().trace("Start processing, anonym debit card");
            if (request == null) {
