...
public class SSRepo_PrepaidApplicationValidation  implements SQLData {
	 private final String SCHEMA_SQL = "OPENGATEV2";
	    private final String PACKAGE_SQL = "OPENGATE_CARD_MANAGEMENT";
	    private final String PROCEDURE_SQL = "prepaidapplicationvalidation";
	    @Getter @Setter
	    
	    private SSEnt_PrepaidAppliValidationRq request;
	    ...
	    public SSRepo_PrepaidApplicationValidation() {
	        logr = new SS_LoggingImpl();
	        logr.setClazz(getClass());
	    }
	 
	    public SSRepo_PrepaidApplicationValidation(SSEnt_PrepaidAppliValidationRq request) {
	        this();
	    ...
	    public void readSQL(SQLInput stream, String typeName) {
	        try {
	            logr.getLog().trace("Start readSQL, prepaid application validation ");
	            response = stream.readString(); // Read output parameter
	        } catch (NullPointerException e) {
	            logr.getLog().trace("No data available in prepaid application validation  : " + e.getMessage());
	        } catch (Exception e) {
	            logr.getLog().error("Error in readSQL, prepaid application validation  ", e);
	        }
	    }
	    ...
	    public void writeSQL(SQLOutput stream) throws SQLException {
	        try {
	            logr.getLog().trace("Start writeSQL, prepaid application validation ");
	            stream.writeString(request != null ? request.toString() : null); 
	            logr.getLog().trace("End writeSQL, prepaid application validation ");
	        } catch (Exception e) {
	            logr.getLog().error("Error in writeSQL, prepaid application validation ", e);
	        }
	    }
	    ...
	    public String processing(DataSource dataSource) {
	        ...
	        try {
	            logr.getLog().trace("Start processing, prepaid application validation ");
	            if (request == null) {
