...
public class SSRepo_ManageAddress implements SQLData {
    ...
    public void readSQL(SQLInput stream, String typeName) {
        try {
            logr.getLog().trace("Start readSQL, Address manage");
            response = stream.readString(); // Read output parameter
        } catch (NullPointerException e) {
            logr.getLog().trace("No data available in Address manage : " + e.getMessage());
        } catch (Exception e) {
            logr.getLog().error("Error in readSQL, Address manage ", e);
        }
    }
    ...
    public void writeSQL(SQLOutput stream) throws SQLException {
        try {
            logr.getLog().trace("Start writeSQL, Address manage");
            stream.writeString(request != null ? request.toString() : null); 
            logr.getLog().trace("End writeSQL, Address manage");
        } catch (Exception e) {
            logr.getLog().error("Error in writeSQL, Address manage", e);
        }
    }
    ...
    public String processing(DataSource dataSource) {
        ...
        try {
            logr.getLog().trace("Start processing, Address manage");
            if (request == null) {
