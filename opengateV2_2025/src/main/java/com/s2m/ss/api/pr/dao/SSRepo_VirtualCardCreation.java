...
public class SSRepo_VirtualCardCreation implements SQLData {
	
	 private final String SCHEMA_SQL = "OPENGATEV2";
	    private final String PACKAGE_SQL = "OPENGATE_CARD_MANAGEMENT";
	    private final String PROCEDURE_SQL = "debitapplication_virtual_card";
	    @Getter @Setter
	    
	    private SSEnt_VirtualCardCreationRq request;
	    ...
	    public SSRepo_VirtualCardCreation() {
	        logr = new SS_LoggingImpl();
	        logr.setClazz(getClass());
	    }
	 
	    public SSRepo_VirtualCardCreation(SSEnt_VirtualCardCreationRq request) {
	        this();
	    ...
	    public void readSQL(SQLInput stream, String typeName) {
	        try {
	            logr.getLog().trace("Start readSQL, debit application virtual card ");
	            response = stream.readString(); // Read output parameter
	        } catch (NullPointerException e) {
	            logr.getLog().trace("No data available in debit application virtual card : " + e.getMessage());
	        } catch (Exception e) {
	            logr.getLog().error("Error in readSQL, debit application virtual card ", e);
	        }
	    }
	    ...
	    public void writeSQL(SQLOutput stream) throws SQLException {
	        try {
	            logr.getLog().trace("Start writeSQL, debit application virtual card ");
	            stream.writeString(request != null ? request.toString() : null); 
	            logr.getLog().trace("End writeSQL, debit application virtual card ");
	        } catch (Exception e) {
	            logr.getLog().error("Error in writeSQL, debit application virtual card ", e);
	        }
	    }
	    ...
	    public String processing(DataSource dataSource) {
	        ...
	        try {
	            logr.getLog().trace("Start processing, debit application virtual card ");
	            if (request == null) {
