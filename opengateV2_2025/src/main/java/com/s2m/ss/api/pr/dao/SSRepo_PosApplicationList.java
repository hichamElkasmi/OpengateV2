...
public class SSRepo_PosApplicationList implements SQLData {

	
	private final String SCHEMA_SQL = "OPENGATEV2";
    private final String PACKAGE_SQL = "opengate_acquirer";
    private final String PROCEDURE_SQL = "posapplication_list";
    @Getter @Setter
    
    private SSEnt_PosApplicationListRq request;
    ...
    public SSRepo_PosApplicationList() {
        logr = new SS_LoggingImpl();
        logr.setClazz(getClass());
    }
 
    public SSRepo_PosApplicationList(SSEnt_PosApplicationListRq request) {
        this();
    ...
    public void readSQL(SQLInput stream, String typeName) {
        try {
            logr.getLog().trace("Start readSQL, pos application list ");
            response = stream.readString(); // Read output parameter
        } catch (NullPointerException e) {
            logr.getLog().trace("No data available in pos application list : " + e.getMessage());
        } catch (Exception e) {
            logr.getLog().error("Error in readSQL, pos application list ", e);
        }
    }
    ...
    public void writeSQL(SQLOutput stream) throws SQLException {
        try {
            logr.getLog().trace("Start writeSQL, pos application list ");
            stream.writeString(request != null ? request.toString() : null); 
            logr.getLog().trace("End writeSQL, pos application list ");
        } catch (Exception e) {
            logr.getLog().error("Error in writeSQL, pos application list ", e);
        }
    }
    ...
    public String processing(DataSource dataSource) {
        ...
        try {
            logr.getLog().trace("Start processing, pos application list ");
            if (request == null) {
