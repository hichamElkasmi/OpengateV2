...
public class SSRepo_CardRisk implements SQLData{
	private final String SCHEMA_SQL = "OPENGATEV2";
    private final String PACKAGE_SQL = "opengate_card_management";
    private final String PROCEDURE_SQL = "GET_CARD_RISK";
    @Getter @Setter
    
    private SSEnt_CardRiskRq request;
    ...
    public SSRepo_CardRisk() {
        logr = new SS_LoggingImpl();
        logr.setClazz(getClass());
    }
 
    public SSRepo_CardRisk(SSEnt_CardRiskRq request) {
        this();
    ...
    public void readSQL(SQLInput stream, String typeName) {
        try {
            logr.getLog().trace("Start readSQL, GET_CARD_RISK");
            response = stream.readString(); // Read output parameter
        } catch (NullPointerException e) {
            logr.getLog().trace("No data available in GET_CARD_RISK: " + e.getMessage());
        } catch (Exception e) {
            logr.getLog().error("Error in readSQL, GET_CARD_RISK", e);
        }
    }
    ...
    public void writeSQL(SQLOutput stream) throws SQLException {
        try {
            logr.getLog().trace("Start writeSQL, get card risk ");
            stream.writeString(request != null ? request.toString() : null); 
            logr.getLog().trace("End writeSQL, GET_CARD_RISK");
        } catch (Exception e) {
            logr.getLog().error("Error in writeSQL, GET_CARD_RISK", e);
        }
    }
    ...
    public String processing(DataSource dataSource) {
        ...
        try {
            logr.getLog().trace("Start processing, get card risk ");
            if (request == null) {
