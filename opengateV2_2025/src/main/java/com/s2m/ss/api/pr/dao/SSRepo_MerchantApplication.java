...
public class SSRepo_MerchantApplication implements SQLData {

	private final String SCHEMA_SQL = "OPENGATEV2";
    private final String PACKAGE_SQL = "opengate_acquirer";
    private final String PROCEDURE_SQL = "merchantapplication";
    @Getter @Setter
    
    private SSEnt_MerchantApplicationRq request;
    ...
    public SSRepo_MerchantApplication() {
        logr = new SS_LoggingImpl();
        logr.setClazz(getClass());
    }
 
    public SSRepo_MerchantApplication(SSEnt_MerchantApplicationRq request) {
        this();
    ...
    public void readSQL(SQLInput stream, String typeName) {
        try {
            logr.getLog().trace("Start readSQL, merchant application");
            response = stream.readString(); // Read output parameter
        } catch (NullPointerException e) {
            logr.getLog().trace("No data available in merchant application : " + e.getMessage());
        } catch (Exception e) {
            logr.getLog().error("Error in readSQL, merchant application ", e);
        }
    }
    ...
    public void writeSQL(SQLOutput stream) throws SQLException {
        try {
            logr.getLog().trace("Start writeSQL, merchant application ");
            stream.writeString(request != null ? request.toString() : null); 
            logr.getLog().trace("End writeSQL, merchant application ");
        } catch (Exception e) {
            logr.getLog().error("Error in writeSQL, merchant application ", e);
        }
    }
    ...
    public String processing(DataSource dataSource) {
        ...
        try {
            logr.getLog().trace("Start processing, merchant application ");
            if (request == null) {
