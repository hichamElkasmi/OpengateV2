...
public class SSRepo_UpdateAccount implements SQLData {
	private final String SCHEMA_SQL = "OPENGATEV2";
    private final String PACKAGE_SQL = "opengate_card_management";
    private final String PROCEDURE_SQL = "update_account";
    @Getter @Setter
    
    private SSEnt_UpdateAccountRq request;
    ...
    public SSRepo_UpdateAccount() {
        logr = new SS_LoggingImpl();
        logr.setClazz(getClass());
    }
 
    public SSRepo_UpdateAccount(SSEnt_UpdateAccountRq request) {
        this();
    ...
    public void readSQL(SQLInput stream, String typeName) {
        try {
            logr.getLog().trace("Start readSQL, update account ");
            response = stream.readString(); // Read output parameter
        } catch (NullPointerException e) {
            logr.getLog().trace("No data available in update account : " + e.getMessage());
        } catch (Exception e) {
            logr.getLog().error("Error in readSQL, update account ", e);
        }
    }
    ...
    public void writeSQL(SQLOutput stream) throws SQLException {
        try {
            logr.getLog().trace("Start writeSQL, update account ");
            stream.writeString(request != null ? request.toString() : null); 
            logr.getLog().trace("End writeSQL, update account ");
        } catch (Exception e) {
            logr.getLog().error("Error in writeSQL, update account ", e);
        }
    }
    ...
    public String processing(DataSource dataSource) {
        ...
        try {
            logr.getLog().trace("Start processing, update account ");
            if (request == null) {
