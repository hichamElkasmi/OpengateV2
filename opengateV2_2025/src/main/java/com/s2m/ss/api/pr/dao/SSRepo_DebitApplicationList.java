...
public class SSRepo_DebitApplicationList implements SQLData {

	private final String SCHEMA_SQL = "OPENGATEV2";
    private final String PACKAGE_SQL = "OPENGATE_CARD_MANAGEMENT";
    private final String PROCEDURE_SQL = "debitapplication_list";
    @Getter @Setter
    
    private SSEnt_DebitApplicationListRq request;
    ...
    public SSRepo_DebitApplicationList() {
        logr = new SS_LoggingImpl();
        logr.setClazz(getClass());
    }
 
    public SSRepo_DebitApplicationList(SSEnt_DebitApplicationListRq request) {
        this();
    ...
    public void readSQL(SQLInput stream, String typeName) {
        try {
            logr.getLog().trace("Start readSQL, debit application list ");
            response = stream.readString(); // Read output parameter
        } catch (NullPointerException e) {
            logr.getLog().trace("No data available in debit application list : " + e.getMessage());
        } catch (Exception e) {
            logr.getLog().error("Error in readSQL, debit application list ", e);
        }
    }
    ...
    public void writeSQL(SQLOutput stream) throws SQLException {
        try {
            logr.getLog().trace("Start writeSQL, debit application list ");
            stream.writeString(request != null ? request.toString() : null); 
            logr.getLog().trace("End writeSQL, debit application list ");
        } catch (Exception e) {
            logr.getLog().error("Error in writeSQL, debit application list ", e);
        }
    }
    ...
    public String processing(DataSource dataSource) {
        ...
        try {
            logr.getLog().trace("Start processing, debit application list ");
            if (request == null) {
