...
public class SSRepo_UpdateMobile implements SQLData {
	private final String SCHEMA_SQL = "OPENGATEV2";
    private final String PACKAGE_SQL = "opengate_card_management";
    private final String PROCEDURE_SQL = "update_mobile";
    @Getter @Setter
    
    private SSEnt_UpdateMobileRq request;
    ...
    public SSRepo_UpdateMobile() {
        logr = new SS_LoggingImpl();
        logr.setClazz(getClass());
    }
 
    public SSRepo_UpdateMobile(SSEnt_UpdateMobileRq request) {
        this();
    ...
    public void readSQL(SQLInput stream, String typeName) {
        try {
            logr.getLog().trace("Start readSQL, update mobile ");
            response = stream.readString(); // Read output parameter
        } catch (NullPointerException e) {
            logr.getLog().trace("No data available in update mobile : " + e.getMessage());
        } catch (Exception e) {
            logr.getLog().error("Error in readSQL, update mobile ", e);
        }
    }
    ...
    public void writeSQL(SQLOutput stream) throws SQLException {
        try {
            logr.getLog().trace("Start writeSQL, update mobile ");
            stream.writeString(request != null ? request.toString() : null); 
            logr.getLog().trace("End writeSQL, update mobile ");
        } catch (Exception e) {
            logr.getLog().error("Error in writeSQL, update mobile ", e);
        }
    }
    ...
    public String processing(DataSource dataSource) {
        ...
        try {
            logr.getLog().trace("Start processing, update mobile ");
            if (request == null) {
