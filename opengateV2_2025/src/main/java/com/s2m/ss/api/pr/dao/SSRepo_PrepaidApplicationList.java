...
public class SSRepo_PrepaidApplicationList implements SQLData {
	
	private final String SCHEMA_SQL = "OPENGATEV2";
    private final String PACKAGE_SQL = "OPENGATE_CARD_MANAGEMENT";
    private final String PROCEDURE_SQL = "prepaidapplication_list";
    @Getter @Setter
    
    private SSEnt_PrepaidApplicationListRq request;
    ...
    public SSRepo_PrepaidApplicationList() {
        logr = new SS_LoggingImpl();
        logr.setClazz(getClass());
    }
 
    public SSRepo_PrepaidApplicationList(SSEnt_PrepaidApplicationListRq request) {
        this();
    ...
    public void readSQL(SQLInput stream, String typeName) {
        try {
            logr.getLog().trace("Start readSQL, prepaid application list ");
            response = stream.readString(); // Read output parameter
        } catch (NullPointerException e) {
            logr.getLog().trace("No data available in prepaid application list : " + e.getMessage());
        } catch (Exception e) {
            logr.getLog().error("Error in readSQL, prepaid application list ", e);
        }
    }
    ...
    public void writeSQL(SQLOutput stream) throws SQLException {
        try {
            logr.getLog().trace("Start writeSQL, prepaid application list ");
            stream.writeString(request != null ? request.toString() : null); 
            logr.getLog().trace("End writeSQL, prepaid application list ");
        } catch (Exception e) {
            logr.getLog().error("Error in writeSQL,  prepaid application list ", e);
        }
    }
    ...
    public String processing(DataSource dataSource) {
        ...
        try {
            logr.getLog().trace("Start processing, prepaid application list ");
            if (request == null) {
