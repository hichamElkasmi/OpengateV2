...
public class SSRepo_MerchantUpdateNoApplication implements SQLData  {

	private final String SCHEMA_SQL = "OPENGATEV2";
    private final String PACKAGE_SQL = "opengate_acquirer";
    private final String PROCEDURE_SQL = "merchantupdate";
    @Getter @Setter
    
    private SSEnt_MerchantUpdateNoApplicationRq request;
    ...
    public SSRepo_MerchantUpdateNoApplication() {
        logr = new SS_LoggingImpl();
        logr.setClazz(getClass());
    }
 
    public SSRepo_MerchantUpdateNoApplication(SSEnt_MerchantUpdateNoApplicationRq request) {
        this();
    ...
    public void readSQL(SQLInput stream, String typeName) {
        try {
            logr.getLog().trace("Start readSQL, merchant update ");
            response = stream.readString(); // Read output parameter
        } catch (NullPointerException e) {
            logr.getLog().trace("No data available in merchant update : " + e.getMessage());
        } catch (Exception e) {
            logr.getLog().error("Error in readSQL, merchant update ", e);
        }
    }
    ...
    public void writeSQL(SQLOutput stream) throws SQLException {
        try {
            logr.getLog().trace("Start writeSQL, merchant update ");
            stream.writeString(request != null ? request.toString() : null); 
            logr.getLog().trace("End writeSQL, merchant update ");
        } catch (Exception e) {
            logr.getLog().error("Error in writeSQL, merchant update", e);
        }
    }
    ...
    public String processing(DataSource dataSource) {
        ...
        try {
            logr.getLog().trace("Start processing, merchant update");
            if (request == null) {
