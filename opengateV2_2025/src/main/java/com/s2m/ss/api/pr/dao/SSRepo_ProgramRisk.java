...
public class SSRepo_ProgramRisk implements SQLData {

	private final String SCHEMA_SQL = "OPENGATEV2";
    private final String PACKAGE_SQL = "opengate_card_management";
    private final String PROCEDURE_SQL = "GET_PROGRAM_RISK";
    @Getter @Setter
    
    private  SSEnt_ProgramRiskRq request;
    ...
    public SSRepo_ProgramRisk() {
        logr = new SS_LoggingImpl();
        logr.setClazz(getClass());
    }
 
    public SSRepo_ProgramRisk(SSEnt_ProgramRiskRq request) {
        this();
    ...
    public void readSQL(SQLInput stream, String typeName) {
        try {
            logr.getLog().trace("Start readSQL, get program risk ");
    ...
    public void writeSQL(SQLOutput stream) throws SQLException {
        try {
            logr.getLog().trace("Start writeSQL, get program risk ");
            stream.writeString(request != null ? request.toString() : null); 
            logr.getLog().trace("End writeSQL, get program risk ");
        } catch (Exception e) {
            logr.getLog().error("Error in writeSQL, get program risk ", e);
        }
    }
    ...
    public String processing(DataSource dataSource) {
        ...
        try {
            logr.getLog().trace("Start processing, get program risk ");
            if (request == null) {
