...
public class SSRepo_SmsAlert  implements SQLData{

	private final String SCHEMA_SQL = "OPENGATEV2";
    private final String PACKAGE_SQL = "opengate_card_management";
    private final String PROCEDURE_SQL = "SEND_SMS_ALERT";
    @Getter @Setter
    
    private SSEnt_SmsAlertRq request;
    ...
    public SSRepo_SmsAlert() {
        logr = new SS_LoggingImpl();
        logr.setClazz(getClass());
    }
 
    public SSRepo_SmsAlert(SSEnt_SmsAlertRq request) {
        this();
    ...
    public void writeSQL(SQLOutput stream) throws SQLException {
        try {
            logr.getLog().trace("Start writeSQL, sms alert ");
            stream.writeString(request != null ? request.toString() : null); 
            logr.getLog().trace("End writeSQL, sms alert ");
        } catch (Exception e) {
            logr.getLog().error("Error in writeSQL, sms alert ", e);
        }
    }
    ...
    public String processing(DataSource dataSource) {
        ...
        try {
            logr.getLog().trace("Start processing, sms alert ");
            if (request == null) {
