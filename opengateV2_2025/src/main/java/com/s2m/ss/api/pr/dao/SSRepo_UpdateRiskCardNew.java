...
public class SSRepo_UpdateRiskCardNew implements SQLData  {
	private final String SCHEMA_SQL = "OPENGATEV2";
    private final String PACKAGE_SQL = "opengate_card_management";
    private final String PROCEDURE_SQL = "update_risk_card_new";
    @Getter @Setter
    
    private SSEnt_UpdateRiskCardNewRq request;
    ...
    public SSRepo_UpdateRiskCardNew() {
        logr = new SS_LoggingImpl();
        logr.setClazz(getClass());
    }
 
    public SSRepo_UpdateRiskCardNew(SSEnt_UpdateRiskCardNewRq request) {
        this();
    ...
    public void readSQL(SQLInput stream, String typeName) {
        try {
            logr.getLog().trace("Start readSQL, update risk card new ");
            response = stream.readString(); // Read output parameter
        } catch (NullPointerException e) {
            logr.getLog().trace("No data available in  update risk card new  : " + e.getMessage());
        } catch (Exception e) {
            logr.getLog().error("Error in readSQL,  update risk card new  ", e);
        }
    }
    ...
    public void writeSQL(SQLOutput stream) throws SQLException {
        try {
            logr.getLog().trace("Start writeSQL,  update risk card new ");
            stream.writeString(request != null ? request.toString() : null); 
            logr.getLog().trace("End writeSQL,  update risk card new  ");
        } catch (Exception e) {
            logr.getLog().error("Error in writeSQL,  update risk card new ", e);
        }
    }
    ...
    public String processing(DataSource dataSource) {
        ...
        try {
            logr.getLog().trace("Start processing,  update risk card new ");
            if (request == null) {
