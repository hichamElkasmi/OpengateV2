...
public class SSRepo_GetMerchantTransactions implements SQLData {
	
	private final String SCHEMA_SQL = "OPENGATEV2";
    private final String PACKAGE_SQL = "opengate_acquirer";
    private final String PROCEDURE_SQL = "get_merchant_transactions";
    @Getter @Setter
    
    private SSEnt_GetMerchantTransactionsRq request;
    ...
    public SSRepo_GetMerchantTransactions() {
        logr = new SS_LoggingImpl();
        logr.setClazz(getClass());
    }
 
    public SSRepo_GetMerchantTransactions(SSEnt_GetMerchantTransactionsRq request) {
        this();
    ...
    public void readSQL(SQLInput stream, String typeName) {
        try {
            logr.getLog().trace("Start readSQL, get_merchant_transactions");
            response = stream.readString(); 
        } catch (NullPointerException e) {
            logr.getLog().trace("No data available get_merchant_transactions: " + e.getMessage());
        } catch (Exception e) {
            logr.getLog().error("Error in readSQL, get_merchant_transactions", e);
        }
    }
    ...
    public void writeSQL(SQLOutput stream) throws SQLException {
        try {
            logr.getLog().trace("Start writeSQL, get_merchant_transactions");
            stream.writeString(request != null ? request.toString() : null); 
            logr.getLog().trace("End writeSQL, get_merchant_transactions");
        } catch (Exception e) {
            logr.getLog().error("Error in writeSQL, get_merchant_transactions", e);
        }
    }
    ...
    public String processing(DataSource dataSource) {
        ...
        try {
            logr.getLog().trace("Start processing, get_merchant_transactions");
            if (request == null) {
