...
public class SSRepo_Renew implements SQLData {

	
	private final String SCHEMA_SQL = "OPENGATEV2";
    private final String PACKAGE_SQL = "OPENGATE_CARD_MANAGEMENT";
    private final String PROCEDURE_SQL = "renew";
    @Getter @Setter
    
    private SSEnt_RenewRq request;
    ...
    public SSRepo_Renew(SSEnt_RenewRq request) {
        this();
    ...
    public void readSQL(SQLInput stream, String typeName) {
        try {
            logr.getLog().trace("Start readSQL, Renew ");
            response = stream.readString(); // Read output parameter
        } catch (NullPointerException e) {
            logr.getLog().trace("No data available in renew : " + e.getMessage());
        } catch (Exception e) {
            logr.getLog().error("Error in readSQL, renew ", e);
        }
    }
    ...
    public void writeSQL(SQLOutput stream) throws SQLException {
        try {
            logr.getLog().trace("Start writeSQL, renew ");
            stream.writeString(request != null ? request.toString() : null); 
            logr.getLog().trace("End writeSQL, renew ");
        } catch (Exception e) {
            logr.getLog().error("Error in writeSQL, renew ", e);
        }
    }
    ...
    public String processing(DataSource dataSource) {
        ...
        try {
            logr.getLog().trace("Start processing, Renew ");
            if (request == null) {
