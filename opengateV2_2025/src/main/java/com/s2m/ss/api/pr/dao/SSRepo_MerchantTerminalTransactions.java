...
public class SSRepo_MerchantTerminalTransactions implements SQLData {

	private final String SCHEMA_SQL = "OPENGATEV2";
    private final String PACKAGE_SQL = "opengate_acquirer";
    private final String PROCEDURE_SQL = "GET_MERCHANT_TERMINAL_TRANSACTIONS";
    @Getter @Setter
    
    private SSEnt_MerchantTerminalTransactionsRq request;
    ...
    public SSRepo_MerchantTerminalTransactions() {
        logr = new SS_LoggingImpl();
        logr.setClazz(getClass());
    }
 
    public SSRepo_MerchantTerminalTransactions(SSEnt_MerchantTerminalTransactionsRq request) {
        this();
    ...
    public void readSQL(SQLInput stream, String typeName) {
        try {
            logr.getLog().trace("Start readSQL, get merchant terminal transactions ");
            response = stream.readString(); // Read output parameter
        } catch (NullPointerException e) {
            logr.getLog().trace("No data available in  get merchant terminal transactions  : " + e.getMessage());
        } catch (Exception e) {
            logr.getLog().error("Error in readSQL,  get merchant terminal transactions ", e);
        }
    }
    ...
    public void writeSQL(SQLOutput stream) throws SQLException {
        try {
            logr.getLog().trace("Start writeSQL,  get merchant terminal transactions ");
            stream.writeString(request != null ? request.toString() : null); 
            logr.getLog().trace("End writeSQL,  get merchant terminal transactions ");
        } catch (Exception e) {
            logr.getLog().error("Error in writeSQL,  get merchant terminal transactions ", e);
        }
    }
    ...
    public String processing(DataSource dataSource) {
        ...
        try {
            logr.getLog().trace("Start processing,  get merchant terminal transactions ");
            if (request == null) {
