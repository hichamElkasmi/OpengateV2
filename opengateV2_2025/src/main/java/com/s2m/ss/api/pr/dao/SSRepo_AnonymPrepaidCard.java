...
public class SSRepo_AnonymPrepaidCard implements SQLData {
	private final String SCHEMA_SQL = "OPENGATEV2";
    private final String PACKAGE_SQL = "opengate_card_management";
    private final String PROCEDURE_SQL = "anonymPrepaidCard";
    @Getter @Setter
    
    private SSEnt_AnonymPrepaidCardRq request;
    ...
    public SSRepo_AnonymPrepaidCard() {
        logr = new SS_LoggingImpl();
        logr.setClazz(getClass());
    }
 
    public SSRepo_AnonymPrepaidCard(SSEnt_AnonymPrepaidCardRq request) {
        this();
    ...
    public void readSQL(SQLInput stream, String typeName) {
        try {
            ...
            logr.getLog().trace("No data available in anonym prepaid card : " + e.getMessage());
        } catch (Exception e) {
    ...
    public void writeSQL(SQLOutput stream) throws SQLException {
        try {
            logr.getLog().trace("Start writeSQL, anonym prepaid card ");
            stream.writeString(request != null ? request.toString() : null); 
            logr.getLog().trace("End writeSQL, change status");
        } catch (Exception e) {
            logr.getLog().error("Error in writeSQL, anonym prepaid card ", e);
        }
    }
    ...
    public String processing(DataSource dataSource) {
        ...
        try {
            logr.getLog().trace("Start processing, anonym prepaid card ");
            if (request == null) {
