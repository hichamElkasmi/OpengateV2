...
public class SSRepo_DebitApplicationValidation implements SQLData {

	
	private final String SCHEMA_SQL = "OPENGATEV2";
    private final String PACKAGE_SQL = "OPENGATE_CARD_MANAGEMENT";
    private final String PROCEDURE_SQL = "debitapplicationvalidation";
    @Getter @Setter
    
    private SSEnt_DebitApplicationValidationRq request;
    ...
    public SSRepo_DebitApplicationValidation() {
        logr = new SS_LoggingImpl();
        logr.setClazz(getClass());
    }
 
    public SSRepo_DebitApplicationValidation(SSEnt_DebitApplicationValidationRq request) {
        this();
    ...
    public void readSQL(SQLInput stream, String typeName) {
        try {
            logr.getLog().trace("Start readSQL, debit application validation ");
            response = stream.readString(); // Read output parameter
        } catch (NullPointerException e) {
            logr.getLog().trace("No data available in debit application validation : " + e.getMessage());
        } catch (Exception e) {
            logr.getLog().error("Error in readSQL, debit application validation ", e);
        }
    }
    ...
    public void writeSQL(SQLOutput stream) throws SQLException {
        try {
            logr.getLog().trace("Start writeSQL, debit application validation ");
            stream.writeString(request != null ? request.toString() : null); 
            logr.getLog().trace("End writeSQL, debit application validation");
        } catch (Exception e) {
            logr.getLog().error("Error in writeSQL, debit application validation ", e);
        }
    }
    ...
    public String processing(DataSource dataSource) {
        ...
        try {
            logr.getLog().trace("Start processing, debit application validation");
            if (request == null) {
