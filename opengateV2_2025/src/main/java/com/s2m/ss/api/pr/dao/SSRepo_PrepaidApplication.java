...
public class SSRepo_PrepaidApplication implements SQLData {

		private final String SCHEMA_SQL = "OPENGATEV2";
	    private final String PACKAGE_SQL = "OPENGATE_CARD_MANAGEMENT";
	    private final String PROCEDURE_SQL = "prepaidapplication";
	    @Getter @Setter
	    
	    private SSEnt_PrepaidApplicationRq request;
	    ...
	    public SSRepo_PrepaidApplication() {
	        logr = new SS_LoggingImpl();
	        logr.setClazz(getClass());
	    }
	 
	    public SSRepo_PrepaidApplication(SSEnt_PrepaidApplicationRq request) {
	        this();
	    ...
	    public void readSQL(SQLInput stream, String typeName) {
	        try {
	            logr.getLog().trace("Start readSQL, prepaid application ");
	            response = stream.readString(); // Read output parameter
	        } catch (NullPointerException e) {
	            logr.getLog().trace("No data available in prepaid application : " + e.getMessage());
	        } catch (Exception e) {
	            logr.getLog().error("Error in readSQL, prepaid application ", e);
	        }
	    }
	    ...
	    public void writeSQL(SQLOutput stream) throws SQLException {
	        try {
	            logr.getLog().trace("Start writeSQL, prepaid application ");
	            stream.writeString(request != null ? request.toString() : null); 
	            logr.getLog().trace("End writeSQL, prepaid application ");
	        } catch (Exception e) {
	            logr.getLog().error("Error in writeSQL, prepaid application ", e);
	        }
	    }
	    ...
	    public String processing(DataSource dataSource) {
	        ...
	        try {
	            logr.getLog().trace("Start processing, prepaid application ");
	            if (request == null) {
