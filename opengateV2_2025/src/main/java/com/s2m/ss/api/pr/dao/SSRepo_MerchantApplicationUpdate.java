...
public class SSRepo_MerchantApplicationUpdate implements SQLData{
	private final String SCHEMA_SQL = "OPENGATEV2";
    private final String PACKAGE_SQL = "opengate_acquirer";
    private final String PROCEDURE_SQL = "merchantapplicationupdate";
    
    @Getter @Setter 
    private SSEnt_MerchantApplicationUpdateRq request;
    ...
    public SSRepo_MerchantApplicationUpdate() {
        logr = new SS_LoggingImpl();
        logr.setClazz(getClass());
    }
 
    public SSRepo_MerchantApplicationUpdate(SSEnt_MerchantApplicationUpdateRq request) {
        this();
    ...
    public void readSQL(SQLInput stream, String typeName) {
        try {
            logr.getLog().trace("Start readSQL, merchant application update ");
            response = stream.readString(); 
        } catch (NullPointerException e) {
            logr.getLog().trace("No data available in merchant application update  : " + e.getMessage());
        } catch (Exception e) {
            logr.getLog().error("Error in readSQL, merchant application update  ", e);
        }
    }
    ...
    public void writeSQL(SQLOutput stream) throws SQLException {
        try {
            logr.getLog().trace("Start writeSQL, merchant application update ");
            stream.writeString(request != null ? request.toString() : null); 
            logr.getLog().trace("End writeSQL, merchant application update ");
        } catch (Exception e) {
            logr.getLog().error("Error in writeSQL, merchant application update ", e);
        }
    }
    ...
    public String processing(DataSource dataSource) {
        ...
        try {
            logr.getLog().trace("Start processing, merchant application update ");
            if (request == null) {
