...
public class SSRepo_ResetPin implements SQLData {

	private final String SCHEMA_SQL = "OPENGATEV2";
    private final String PACKAGE_SQL = "OPENGATE_CARD_MANAGEMENT";
    private final String PROCEDURE_SQL = "reset_pin";
    @Getter @Setter
    
    private SSEnt_ResetPinRq request;
    ...
    public SSRepo_ResetPin() {
        logr = new SS_LoggingImpl();
        logr.setClazz(getClass());
    }
 
    public SSRepo_ResetPin(SSEnt_ResetPinRq request) {
        this();
    ...
    public void readSQL(SQLInput stream, String typeName) {
        try {
            logr.getLog().trace("Start readSQL, reset pin ");
            response = stream.readString(); // Read output parameter
        } catch (NullPointerException e) {
            logr.getLog().trace("No data available in reset pin : " + e.getMessage());
        } catch (Exception e) {
            logr.getLog().error("Error in readSQL, reset pin ", e);
        }
    }
    ...
    public void writeSQL(SQLOutput stream) throws SQLException {
        try {
            logr.getLog().trace("Start writeSQL, reset pin ");
            stream.writeString(request != null ? request.toString() : null); 
            logr.getLog().trace("End writeSQL, reset pin ");
        } catch (Exception e) {
            logr.getLog().error("Error in writeSQL, reset pin ", e);
        }
    }
    ...
    public String processing(DataSource dataSource) {
        ...
        try {
            logr.getLog().trace("Start processing, reset pin ");
            if (request == null) {
