...
public class SSRepo_StatusMerchant implements SQLData {

	private final String SCHEMA_SQL = "OPENGATEV2";
    private final String PACKAGE_SQL = "opengate_acquirer";
    private final String PROCEDURE_SQL = "update_merchant_status";
    @Getter @Setter
    
    private SSEnt_StatusMerchantRq request;
    ...
    public SSRepo_StatusMerchant() {
        logr = new SS_LoggingImpl();
        logr.setClazz(getClass());
    }
 
    public SSRepo_StatusMerchant(SSEnt_StatusMerchantRq request) {
        this();
    ...
    public void readSQL(SQLInput stream, String typeName) {
        try {
            logr.getLog().trace("Start readSQL, status merchant ");
            response = stream.readString(); 
        } catch (NullPointerException e) {
            logr.getLog().trace("No data available in statut merchant : " + e.getMessage());
        } catch (Exception e) {
            logr.getLog().error("Error in readSQL, statut merchant ", e);
        }
    }
    ...
    public void writeSQL(SQLOutput stream) throws SQLException {
        try {
            logr.getLog().trace("Start writeSQL, status merchant ");
            stream.writeString(request != null ? request.toString() : null); 
            logr.getLog().trace("End writeSQL, status merchant ");
        } catch (Exception e) {
            logr.getLog().error("Error in writeSQL, status merchant ", e);
        }
    }
    ...
    public String processing(DataSource dataSource) {
        ...
        try {
            logr.getLog().trace("Start processing, status merchant ");
            if (request == null) {
