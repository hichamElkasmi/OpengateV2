...
public class SSrepo_Repository implements SQLData {
	    ...
	    public void readSQL(SQLInput stream, String typeName) {
	        try {
	            logr.getLog().trace("Start readSQL, get repository");
	            response = stream.readString(); 
	        } catch (NullPointerException e) {
	            logr.getLog().trace("No data available in get repository: " + e.getMessage());
	        } catch (Exception e) {
	            logr.getLog().error("Error in readSQL, get repository", e);
	        }
	    }
