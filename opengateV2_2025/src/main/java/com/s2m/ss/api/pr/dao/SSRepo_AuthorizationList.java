...
public class SSRepo_AuthorizationList implements SQLData {
	private final String SCHEMA_SQL = "OPENGATEV2";
    private final String PACKAGE_SQL = "opengate_card_management";
    private final String PROCEDURE_SQL = "authorization_list";
    @Getter @Setter
    
    private SSEnt_AuthorizationListRq request;
    ...
    public SSRepo_AuthorizationList() {
        logr = new SS_LoggingImpl();
        logr.setClazz(getClass());
    }
 
    public SSRepo_AuthorizationList(SSEnt_AuthorizationListRq request) {
        this();
    ...
    public void readSQL(SQLInput stream, String typeName) {
        try {
            logr.getLog().trace("Start readSQL, authorization list");
            response = stream.readString(); // Read output parameter
        } catch (NullPointerException e) {
            logr.getLog().trace("No data available  " + e.getMessage());
        } catch (Exception e) {
            logr.getLog().error("Error in readSQL, authorization list :", e);
        }
    }
    ...
    public void writeSQL(SQLOutput stream) throws SQLException {
        try {
            logr.getLog().trace("Start writeSQL, authorization list ");
            stream.writeString(request != null ? request.toString() : null); 
            logr.getLog().trace("End writeSQL, change status");
        } catch (Exception e) {
            logr.getLog().error("Error in writeSQL, authorization list :", e);
        }
    }
    ...
    public String processing(DataSource dataSource) {
        ...
        try {
            logr.getLog().trace("Start processing, authorization list ");
            if (request == null) {
