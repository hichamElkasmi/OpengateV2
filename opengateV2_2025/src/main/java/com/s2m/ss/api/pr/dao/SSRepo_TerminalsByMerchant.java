...
public class SSRepo_TerminalsByMerchant implements SQLData {

	private final String SCHEMA_SQL = "OPENGATEV2";
    private final String PACKAGE_SQL = "opengate_acquirer";
    private final String PROCEDURE_SQL = "GET_TERMINARLS_BY_MERCHANT_ID";
    @Getter @Setter
    
    private SSEnt_TerminalsByMerchantRq request;
    ...
    public SSRepo_TerminalsByMerchant() {
        logr = new SS_LoggingImpl();
        logr.setClazz(getClass());
    }
 
    public SSRepo_TerminalsByMerchant(SSEnt_TerminalsByMerchantRq request) {
        this();
    ...
    public void writeSQL(SQLOutput stream) throws SQLException {
        try {
            logr.getLog().trace("Start writeSQL, get terminals by merchant id ");
            stream.writeString(request != null ? request.toString() : null); 
            logr.getLog().trace("End writeSQL, get terminals by merchant id ");
        } catch (Exception e) {
            logr.getLog().error("Error in writeSQL, get terminals by merchant id ", e);
        }
    }
    ...
    public String processing(DataSource dataSource) {
        ...
        try {
            logr.getLog().trace("Start processing, get terminals by merchant id ");
            if (request == null) {
