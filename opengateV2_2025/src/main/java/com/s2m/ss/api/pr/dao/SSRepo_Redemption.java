...
public class SSRepo_Redemption  implements SQLData {
	private final String SCHEMA_SQL = "OPENGATEV2";
    private final String PACKAGE_SQL = "OPENGATE_CARD_MANAGEMENT";
    private final String PROCEDURE_SQL = "redemption";
    @Getter @Setter
    
    private SSEnt_RedemptionRq request;
    ...
    public SSRepo_Redemption() {
        logr = new SS_LoggingImpl();
        logr.setClazz(getClass());
    }
 
    public SSRepo_Redemption(SSEnt_RedemptionRq request) {
        this();
    ...
    public void readSQL(SQLInput stream, String typeName) {
        try {
            logr.getLog().trace("Start readSQL, Redemption ");
            response = stream.readString(); // Read output parameter
        } catch (NullPointerException e) {
            logr.getLog().trace("No data available in Redemption : " + e.getMessage());
        } catch (Exception e) {
            logr.getLog().error("Error in readSQL, Redemption ", e);
        }
    }
    ...
    public void writeSQL(SQLOutput stream) throws SQLException {
        try {
            logr.getLog().trace("Start writeSQL, Redemption  ");
            stream.writeString(request != null ? request.toString() : null); 
            logr.getLog().trace("End writeSQL, Redemption ");
        } catch (Exception e) {
            logr.getLog().error("Error in writeSQL, Redemption ", e);
        }
    }
    ...
    public String processing(DataSource dataSource) {
        ...
        try {
            logr.getLog().trace("Start processing, Redemption ");
            if (request == null) {
