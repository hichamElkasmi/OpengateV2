...
public class SSRepo_AccountList implements SQLData {

	private final String SCHEMA_SQL = "OPENGATEV2";
    private final String PACKAGE_SQL = "opengate_card_management";
    private final String PROCEDURE_SQL = "account_list";
    @Getter @Setter
    
    private SSEnt_AccountListRq request;
    ...
    public SSRepo_AccountList() {
        logr = new SS_LoggingImpl();
        logr.setClazz(getClass());
    }
 
    public SSRepo_AccountList(SSEnt_AccountListRq request) {
        this();
    ...
    public void readSQL(SQLInput stream, String typeName) {
        try {
            logr.getLog().trace("Start readSQL, account list");
            response = stream.readString(); // Read output parameter
        } catch (NullPointerException e) {
            logr.getLog().trace("No data available in account list: " + e.getMessage());
        } catch (Exception e) {
            logr.getLog().error("Error in readSQL, account list", e);
        }
    }
    ...
    public void writeSQL(SQLOutput stream) throws SQLException {
        try {
            logr.getLog().trace("Start writeSQL, account list");
            stream.writeString(request != null ? request.toString() : null); 
            logr.getLog().trace("End writeSQL, account list");
        } catch (Exception e) {
            logr.getLog().error("Error in writeSQL, account list", e);
        }
    }
    ...
    public String processing(DataSource dataSource) {
        ...
        try {
            logr.getLog().trace("Start processing, account list");
            if (request == null) {
