...
public class SSRepo_PosMonitoring implements SQLData {

	private final String SCHEMA_SQL = "OPENGATEV2";
    private final String PACKAGE_SQL = "opengate_acquirer";
    private final String PROCEDURE_SQL = "GET_POS_MONITORING";
    @Getter @Setter
    
    private SSEnt_PosMonitoringRq request;
    ...
    public SSRepo_PosMonitoring() {
        logr = new SS_LoggingImpl();
        logr.setClazz(getClass());
    }
 
    public SSRepo_PosMonitoring(SSEnt_PosMonitoringRq request) {
        this();
    ...
    public void readSQL(SQLInput stream, String typeName) {
        try {
            logr.getLog().trace("Start readSQL, get pos monitoring ");
            response = stream.readString(); // Read output parameter
        } catch (NullPointerException e) {
            logr.getLog().trace("No data available in get pos monitoring : " + e.getMessage());
        } catch (Exception e) {
            logr.getLog().error("Error in readSQL, get pos monitoring ", e);
        }
    }
    ...
    public void writeSQL(SQLOutput stream) throws SQLException {
        try {
            logr.getLog().trace("Start writeSQL, get pos monitoring ");
            stream.writeString(request != null ? request.toString() : null); 
            logr.getLog().trace("End writeSQL, get pos monitoring ");
        } catch (Exception e) {
            logr.getLog().error("Error in writeSQL, get pos monitoring ", e);
        }
    }
    ...
    public String processing(DataSource dataSource) {
        ...
        try {
            logr.getLog().trace("Start processing, get pos monitoring ");
            if (request == null) {
