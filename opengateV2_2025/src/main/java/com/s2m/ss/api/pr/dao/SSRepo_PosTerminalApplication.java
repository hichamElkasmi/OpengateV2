...
public class SSRepo_PosTerminalApplication implements SQLData {

	private final String SCHEMA_SQL = "OPENGATEV2";
    private final String PACKAGE_SQL = "opengate_acquirer";
    private final String PROCEDURE_SQL = "posterminal_posapplication";
    @Getter @Setter
    
    private SSEnt_PosTerminalApplicationRq request;
    ...
    public SSRepo_PosTerminalApplication() {
        logr = new SS_LoggingImpl();
        logr.setClazz(getClass());
    }
 
    public SSRepo_PosTerminalApplication(SSEnt_PosTerminalApplicationRq request) {
        this();
    ...
    public void readSQL(SQLInput stream, String typeName) {
        try {
            logr.getLog().trace("Start readSQL, pos terminal application ");
            response = stream.readString(); // Read output parameter
        } catch (NullPointerException e) {
            logr.getLog().trace("No data available in pos terminal application : " + e.getMessage());
        } catch (Exception e) {
            logr.getLog().error("Error in readSQL, pos terminal application ", e);
        }
    }
    ...
    public void writeSQL(SQLOutput stream) throws SQLException {
        try {
            logr.getLog().trace("Start writeSQL, pos terminal application ");
            stream.writeString(request != null ? request.toString() : null); 
            logr.getLog().trace("End writeSQL, pos terminal application ");
        } catch (Exception e) {
            logr.getLog().error("Error in writeSQL, pos terminal application ", e);
        }
    }
    ...
    public String processing(DataSource dataSource) {
        ...
        try {
            logr.getLog().trace("Start processing, pos terminal application ");
            if (request == null) {
