...
public class SSRepo_PosTerminalList  implements SQLData {
	private final String SCHEMA_SQL = "OPENGATEV2";
    private final String PACKAGE_SQL = "opengate_acquirer";
    private final String PROCEDURE_SQL = "posterminal_list";
    @Getter @Setter
    
    private SSEnt_PosTerminalListRq request;
    ...
    public SSRepo_PosTerminalList() {
        logr = new SS_LoggingImpl();
        logr.setClazz(getClass());
    }
 
    public SSRepo_PosTerminalList(SSEnt_PosTerminalListRq request) {
        this();
    ...
    public void readSQL(SQLInput stream, String typeName) {
        try {
            logr.getLog().trace("Start readSQL, pos terminal list ");
            response = stream.readString(); 
        } catch (NullPointerException e) {
            logr.getLog().trace("No data available in pos terminal list : " + e.getMessage());
        } catch (Exception e) {
            logr.getLog().error("Error in readSQL, pos terminal list ", e);
        }
    }
    ...
    public void writeSQL(SQLOutput stream) throws SQLException {
        try {
            logr.getLog().trace("Start writeSQL, pos terminal list ");
            stream.writeString(request != null ? request.toString() : null); 
            logr.getLog().trace("End writeSQL, pos terminal list ");
        } catch (Exception e) {
            logr.getLog().error("Error in writeSQL, pos terminal list  ", e);
        }
    }
    ...
    public String processing(DataSource dataSource) {
        ...
        try {
            logr.getLog().trace("Start processing, pos terminal list ");
            if (request == null) {
