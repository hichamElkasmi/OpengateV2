...
public class SSRepo_SendPinBySms implements SQLData {

	private final String SCHEMA_SQL = "OPENGATEV2";
    private final String PACKAGE_SQL = "opengate_card_management";
    private final String PROCEDURE_SQL = "SEND_PIN_BY_SMS";
    @Getter @Setter
    
    private SSEnt_SendPinBySmsRq request;
    ...
    public SSRepo_SendPinBySms() {
        logr = new SS_LoggingImpl();
        logr.setClazz(getClass());
    }
 
    public SSRepo_SendPinBySms(SSEnt_SendPinBySmsRq request) {
        this();
    ...
    public void writeSQL(SQLOutput stream) throws SQLException {
        try {
            logr.getLog().trace("Start writeSQL, send pin by sms ");
            stream.writeString(request != null ? request.toString() : null); 
            logr.getLog().trace("End writeSQL, send pin by sms ");
        } catch (Exception e) {
            logr.getLog().error("Error in writeSQL, send pin by sms ", e);
        }
    }
    ...
    public String processing(DataSource dataSource) {
        ...
        try {
            logr.getLog().trace("Start processing, send pin by sms ");
            if (request == null) {
